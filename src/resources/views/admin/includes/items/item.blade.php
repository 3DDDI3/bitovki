<div class="input_block {{ $table == 'catalog_chars_value' ? 'input_block_line' : '' }}">
    ...
    <input class="items-edit" type="text" name="items[]" value="{{ $item_value ?? '' }}"
        data-id="{{ $item_id ?? 0 }}"
        data-table="{{ $table ?? '' }}"
        data-type="{{ $type ?? '' }}" autocomplete="off" placeholder="{{ $placeholder ?? '' }}">

    @if (!empty($second))
        <input class="items-edit2" type="text" name="items2[]" value="{{ $item_value2 ?? '' }}"
            data-id="{{ $item_id ?? 0 }}"
            data-table="{{ $table ?? '' }}"
            data-type="{{ $type ?? '' }}" autocomplete="off" placeholder="{{ $placeholder2 ?? '' }}">
    @endif
