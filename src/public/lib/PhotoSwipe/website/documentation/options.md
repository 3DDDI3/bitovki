...
In non-dialog mode, the template's position relative to the viewport should be subtracted from `x` and `y`. Look at [the FAQ](faq.html#inline-gallery) for more information.
...
### `dialog` <code class="default">boolean</code> <code class="default">true</code>
...
// 
// functions are triggered after click on button that opens share dialog,
