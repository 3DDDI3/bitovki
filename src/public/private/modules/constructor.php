...
if (isset($_POST['saveBlock'])) {
    ...
    if (!$object) {
        $_SESSION['notice'] = 'Такой статьи не существует';
            ...
            if ($blockItem) {
                switch ($blockPosition->block_id) {
                    ...
                    case 2:
                        ...
                        isset($_POST['share_show']) ? $blockItem->share_show = 1 : $blockItem->share_show = 0;
                        $blockItem->save();
                        break;
                    case 3:
    ...
    header('Location: /admin/constructor?editBlock&id=' . $object->id . '&position_id=' . $blockPosition->id);
    exit;
}
...
if (isset($_GET['id']) && !empty($_GET['id'])):
    ...
    if (isset($_GET['addBlock'])) {
        ...
        header('Location: /admin/constructor?id=' . $object->id);
        exit;
                    ...
                    <form action='/<?= URI ?>' method='post' class='admin_edit-form' enctype='multipart/form-data'>
                        <?php switch ($blockPosition->block_id):
                            case 1:
                                ...
                                break;
                            case 2:
                                echo GenerateForm::makeCheckbox('share_show', ($blockItem->share_show == 1) ? true : false, 'Показывать автора и кнопки поделиться');
        ...
        <h1>Содежримое статьи: <?= $object->name ?></h1>
